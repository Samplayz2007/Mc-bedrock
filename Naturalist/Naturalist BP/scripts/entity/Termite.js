...
system.afterEvents.scriptEventReceive.subscribe(e => {
    ...
    if (e.id == "sf_nba:termite_chew") {
        if (entity.typeId != "sf_nba:termite")
		{
			world.sendMessage("Error: sf_nba:termite_chew can only be called from a termite.");
			return;
		}
	...
	for (const candidate of Object.keys(logMap))
	{
		for (const axis of axisTypes)
		{
			if (block.permutation.matches(`${candidate}`, {pillar_axis: axis}))
			{
				...
				world.scoreboard.getObjective('sf_nba:jig_computer.addon_stats')?.addScore('sf_nba:termite_logs', 1);
				return true;
			}
		}
	}
